...
import insane96mcp.progressivebosses.mixin.accessor.DragonSittingFlamingPhaseAccessor;
...
public abstract class DragonSittingScanningPhaseMixin extends AbstractDragonPhaseInstance {
	...
	public int progressivebosses$scanningIdleTime(int original) {
		if (((DragonSittingFlamingPhaseAccessor) this.dragon.getPhaseManager().getPhase(EnderDragonPhase.SITTING_FLAMING)).getFlameCount() == 0)
			return original;
